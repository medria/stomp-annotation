...
st.info(
    "Each annotator should open this Streamlit link in their own browser "
    "window or device, enter their own name, and choose their assigned batch. "
    "Different browser sessions are handled separately."
)
...
with st.sidebar.expander("Batch information"):
    st.write(
        "This batch was constructed to contain a similar proportion "
        "of known Yes articles and articles from the remaining mixed pool."
    )
    st.write(
        f"Batch size: {len(articles)} articles"
    )
